...
export const BubbleMapVisualization: React.FC<BubbleMapVisualizationProps> = ({
  ...
  const createBubbleLinks = (nodes: BubbleNode[]): BubbleLink[] => {
    return connections
  ...
  useEffect(() => {
    ...
    const links = createBubbleLinks(nodes);
    ...
    const simulation = d3.forceSimulation<BubbleNode>(nodes)
      ...
      .force('charge', d3.forceManyBody().strength(d => d.type === 'main' ? -800 : -400))
    ...
    node.on('mouseover', (event, d) => {
      ...
      if (page) {
        ...
        tooltip.html(`
          ...
          ${page.description ? `<div style="font-size: 11px; color: #888; margin-top: 4px; max-width: 200px;">${page.description}</div>` : ''}
        `)
